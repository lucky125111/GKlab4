	...
	SubShader
	{
		Pass
		{
			...
			vertexOutput vert(vertexInput input)
			{
				...
				float3 reflectionVector = reflect(-lightDirection, normalDirection);

				// z wykladu v * r czyli v - unit vecgtor w strone ogladacza, r - z punktu po
				float3 vr = dot(reflectionVector, viewDirection);

				float3 specularReflection = _LightColor0.rgb * _SpecColor.rgb * pow(max(0.0, vr), _Shininess);
